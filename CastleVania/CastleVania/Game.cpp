...
void Game::InitKeyboard()
{
	...
	if (hr != DI_OK)
	{
		DebugOut((wchar_t *)"[ERROR] DirectInput8Create failed!\n");
		return;
	}
	...
	if (hr != DI_OK)
	{
		DebugOut((wchar_t *)"[ERROR] CreateDevice failed!\n");
		return;
	}
	...
	if (hr != DI_OK)
	{
		DebugOut((wchar_t *)"[ERROR] DINPUT8::Acquire failed!\n");
		return;
	}


	DebugOut((wchar_t *)"[INFO] Keyboard has been initialized successfully\n");
}
...
void Game::ProcessKeyboard()
{
	...
	if (FAILED(hr))
	{
		...
		if ((hr == DIERR_INPUTLOST) || (hr == DIERR_NOTACQUIRED))
		{
			...
			if (h == DI_OK)
			{
				DebugOut((wchar_t *)"[INFO] Keyboard re-acquired!\n");
			}
			else
				return;
		}
		...
		{
			DebugOut((wchar_t *)"[ERROR] DINPUT::GetDeviceState failed. Error: %d\n", hr);
			return;
		}
	}
	...
	if (FAILED(hr))
	{
		DebugOut((wchar_t *)"[ERROR] DINPUT::GetDeviceData failed. Error: %d\n", hr);
		return;
	}
